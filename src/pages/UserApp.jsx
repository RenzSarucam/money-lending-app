...
import { calcBorrower, fmt, getEndDate } from "../utils/calc";
...
export default function UserApp() {
  ...
  return (
    <div className="user-shell" style={{ fontFamily: "'Segoe UI', sans-serif", background: "#0d1117", color: "#e6edf3" }}>
      <div className="user-content">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 0 16px" }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: "#f0b429", letterSpacing: 1, margin: 0 }}>💰 My Loan</h1>
            <p style={{ fontSize: 14, color: "#8b949e", marginTop: 4 }}>{session.user.email}</p>
          </div>
          <button onClick={signOut} style={{ ...btnStyle, background: "transparent", border: "1px solid #30363d", color: "#8b949e", fontSize: 14 }}>Logout</button>
        </div>

        {!borrowers.length ? (
          <div style={{ textAlign: "center", padding: "48px 20px", color: "#8b949e" }}>
            <div style={{ fontSize: 50, marginBottom: 12 }}>📭</div>
            <p style={{ fontSize: 16 }}>No loan account linked here yet. Ask your lender to link your account using this email.</p>
          </div>
        ) : (
            ...
            return (
              <div key={b.id} style={cardStyle}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 700 }}>{b.loan_type === "paluwagan" ? "🤝 Paluwagan" : "☀️ Arawan"} Loan</div>
                ...
                {[
                  ...
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 15, margin: "4px 0" }}>
                ...
                <div style={{ textAlign: "right", fontSize: 13, color: "#8b949e", marginBottom: 10 }}>{c.progress.toFixed(1)}% done</div>

                <div style={{ fontSize: 13, fontWeight: 700, color: "#8b949e", textTransform: "uppercase", letterSpacing: 1, margin: "12px 0 8px" }}>
                  Payment History
                </div>
                {sortedPayments.length === 0 ? (
                  <p style={{ color: "#8b949e", fontSize: 15, textAlign: "center", padding: 12 }}>No payments recorded yet</p>
                ) : (
